...
.badge-genuine{background:#dcfce7;color:#15803d;border:1px solid #86efac;border-radius:50px;padding:0.15rem 0.7rem;font-size:0.73rem;font-weight:700;}
.badge-fake{background:#fee2e2;color:#dc2626;border:1px solid #fca5a5;border-radius:50px;padding:0.15rem 0.7rem;font-size:0.73rem;font-weight:700;}
.badge-irr{background:#fef9c3;color:#a16207;border:1px solid #fde047;border-radius:50px;padding:0.15rem 0.7rem;font-size:0.73rem;font-weight:600;}
.alert-fake{background:#fff1f2;border-left:4px solid #dc2626;border-radius:8px;padding:0.9rem 1.1rem;margin-bottom:1rem;color:#7f1d1d;font-size:0.91rem;}
.alert-irr{background:#fffbeb;border-left:4px solid #d97706;border-radius:8px;padding:0.9rem 1.1rem;margin-bottom:1rem;color:#78350f;font-size:0.91rem;}
.alert-genuine{background:#f0fdf4;border-left:4px solid #16a34a;border-radius:8px;padding:0.9rem 1.1rem;margin-bottom:1rem;color:#14532d;font-size:0.91rem;}
.card-fake{border-left:4px solid #dc2626 !important;}
.card-irr{border-left:4px solid #d97706 !important;}
.card-genuine{border-left:4px solid #16a34a !important;}
...
def post_job(cid,title,jtype,loc,salary,exp,deadline,desc,req,mobile):
    # Runs detect_fake_job() → transform_single() from tfidf_features.py
    #                        → PAC model from pac_model.pkl (train_model.py)
    ml_label, ml_conf = detect_fake_job({
        "title":title, "description":desc, "requirements":req,
        "salary_range":salary, "location":loc,
        "company_profile":"", "industry":jtype,
    })
    conn=get_conn()
    conn.execute(
        ...
        (cid,title,jtype,loc,salary,exp,str(deadline) if deadline else None,
...
# ─────────────────────────────────────────────────────────────────────────────
# ML ENGINE  —  connects to the three pipeline files
#
#  Stage 1: data_preprocessing.py  → preprocess()
#           Input : final_balanced_fake_job_postings.csv
#           Output: cleaned_jobs.csv  (stored in DB)
#
#  Stage 2: tfidf_features.py      → extract_features(), transform_single()
#           Input : cleaned_jobs.csv
#           Output: tfidf_vectorizer.pkl, features.npz, meta_features.npy
#
#  Stage 3: train_model.py         → load_data(), add_irrelevant_class()
#           Input : features.npz + meta_features.npy
#           Output: pac_model.pkl, scaler.pkl
#           Labels: 0 = Genuine  1 = Fake  2 = Irrelevant
#
# UI symbols:
#   ✅ Genuine    green  — real job, title matches description
#   🚨 Fake       red    — scam signals detected
#   ⚠️ Irrelevant amber  — title and description are mismatched
# ─────────────────────────────────────────────────────────────────────────────

_HERE = os.path.dirname(os.path.abspath(__file__))

# Artifact paths (output of pipeline files)
_VEC_PATH    = os.path.join(_HERE, "tfidf_vectorizer.pkl")
_MODEL_PATH  = os.path.join(_HERE, "pac_model.pkl")
_SCALER_PATH = os.path.join(_HERE, "scaler.pkl")
_META_PATH   = os.path.join(_HERE, "meta_features.npy")

# PAC label → display info map
_LABEL_MAP = {0: "genuine", 1: "fake", 2: "irrelevant"}
_BADGE = {
    "genuine":    "✅ Genuine",
    "fake":       "🚨 Fake Job",
...
@st.cache_resource(show_spinner="📂 Stage 1 — Preprocessing dataset (data_preprocessing.py)…")
def _run_preprocessing():
    """Calls preprocess() from data_preprocessing.py.
    Returns cleaned DataFrame or None if raw CSV is missing."""
    raw_csv = os.path.join(_HERE, "final_balanced_fake_job_postings.csv")
    if not os.path.exists(raw_csv):
        return None
    return _dp.preprocess()


@st.cache_resource(show_spinner="🔢 Stage 2 — TF-IDF features (tfidf_features.py)…")
def _run_tfidf(cleaned_df):
    """Calls extract_features() from tfidf_features.py.
    Returns (vectorizer, X, meta_cols, y) or (None,None,[],None) if no data."""
    if cleaned_df is None:
        return None, None, [], None
    X, y, vectorizer, meta_cols = _tf.extract_features(cleaned_df)
    return vectorizer, X, meta_cols, y


@st.cache_resource(show_spinner="🤖 Stage 3 — Training PAC model (train_model.py)…")
def _run_training(cleaned_df, X, y):
    """Calls add_irrelevant_class() from train_model.py, then trains + saves model.
    Returns (model, scaler) or (None, None) if no data."""
    if cleaned_df is None or X is None:
        return None, None
    from scipy.sparse import vstack, csr_matrix
    from sklearn.preprocessing import MaxAbsScaler
    from sklearn.linear_model import SGDClassifier

    X_aug, y_aug = _tm.add_irrelevant_class(cleaned_df, X, ratio=0.05)
    scaler = MaxAbsScaler()
    X_scaled = scaler.fit_transform(X_aug)
    model = SGDClassifier(loss="hinge", penalty=None, learning_rate="pa1",
                          eta0=1.0, max_iter=1000, random_state=42, tol=1e-3)
...
def _load_artifacts():
    """Loads saved artifacts if they exist, else runs the full pipeline.
    Returns (vectorizer, scaler, model, meta_cols)."""
    artifacts_ready = all(os.path.exists(p) for p in
                          [_VEC_PATH, _MODEL_PATH, _SCALER_PATH, _META_PATH])
    if artifacts_ready:
        vectorizer = joblib.load(_VEC_PATH)
        scaler     = joblib.load(_SCALER_PATH)
        model      = joblib.load(_MODEL_PATH)
        meta_cols  = list(np.load(_META_PATH, allow_pickle=True))
        return vectorizer, scaler, model, meta_cols

    # Run full pipeline
    cleaned_df = _run_preprocessing()
    vectorizer, X, meta_cols, y = _run_tfidf(cleaned_df)
    model, scaler = _run_training(cleaned_df, X, y)
    return vectorizer, scaler, model, meta_cols


def detect_fake_job(text_dict: dict) -> tuple:
    """Run PAC model on a job dict via transform_single() from tfidf_features.py.
    Returns (label_str, confidence_pct).
    label_str ∈ {genuine, fake, irrelevant, pending}"""
    vectorizer, scaler, model, meta_cols = _load_artifacts()
    if model is None:
        return "pending", 0.0
    # transform_single() is from tfidf_features.py — called directly
    X = _tf.transform_single(text_dict, vectorizer, meta_cols)
    X_scaled = scaler.transform(X)
    pred_int = int(model.predict(X_scaled)[0])
    label    = _LABEL_MAP.get(pred_int, "pending")
    try:
        scores = model.decision_function(X_scaled).flatten()
        s      = np.sort(scores)[::-1]
        margin = float(s[0] - s[1]) if len(s) > 1 else float(s[0])
        conf   = round(min(100.0, max(10.0, 50.0 + margin * 10.0)), 1)
    except Exception:
        conf = 0.0
    return label, conf


@st.cache_resource(show_spinner="🤖 Building job-matching engine…")
def build_ml_engine():
    """Builds RAW_JOBS DataFrame + TF-IDF cosine-similarity index for seeker matching.
    Also runs detect_fake_job() on each row using the PAC model.
    Returns (df, tfidf, tfidf_matrix)."""
    ...
    # Run PAC fraud detection on every row
    ml_labels, ml_confs = [], []
    for _, row in df.iterrows():
        lbl, conf = detect_fake_job({
            "title": row["title"], "description": row["description"],
            "requirements": "", "salary_range": row["salary"],
            "location": row["location"], "industry": row["industry"],
            "company_profile": row["company"],
        })
        ml_labels.append(lbl)
        ml_confs.append(conf)
    df["ml_label"] = ml_labels
    df["ml_conf"]  = ml_confs

    # Cosine-similarity index for seeker recommendations
    df["corpus"] = (df["title"] + " " +
                    df["skills"].str.replace(",", " ") + " " +
                    df["description"])
    tfidf = _TFIDF(max_features=6000, ngram_range=(1,2),
...
def recommend_jobs(seeker_text: str, df, tfidf, tfidf_matrix, top_n=5):
    """Top-N jobs by cosine similarity to seeker profile text."""
...
def page_home():
    ...
    if user and user["role"] == "seeker":
        ...
        if profile_text:
            ...
            st.markdown(f"""
            <div class="ai-box">
              🤖 <b>AI Recommendations:</b> Top verified jobs matching your profile:
            </div>
            """, unsafe_allow_html=True)
            cols = st.columns(3)
...
def page_jobs():
    ...
    for _, row in results.iterrows():
        ...
        conf = row.get("ml_conf", 0) or 0
        conf_str = f" ({conf:.0f}%)" if conf > 0 else ""
        badge_css = {"genuine":"badge-genuine","fake":"badge-fake","irrelevant":"badge-irr"}.get(lbl,"badge-ai")
        badge_txt = _BADGE.get(lbl, "⏳")
        card_css  = {"genuine":"card-genuine","fake":"card-fake","irrelevant":"card-irr"}.get(lbl,"")
        skills_preview = " ".join(f"<span class='tag'>{s.strip()}</span>" for s in str(row["skills"]).split(",")[:4])
        st.markdown(f"""
        <div class="job-card {card_css}">
          <h3>{row['title']}</h3>
          <div class="company">{row['company']} · {row['location']}</div>
          <span class="tag-accent">{row['salary']}</span>
          <span class="tag">{row['job_type']}</span>
          <span class="tag">{row['exp']} yrs exp</span>
          <span class="{badge_css}">{badge_txt}{conf_str}</span>
          <span class="badge-ai">🤖 {pct}% match</span>
...
def render_dataset_job_detail(row, idx):
    ...
    # ML alert banner — uses PAC model output from train_model.py
    if lbl == "fake":
        st.markdown(f'<div class="alert-fake">🚨 <b>AI Fraud Alert:</b> Our PAC model '
                    f'(train_model.py) flagged this job as <b>Fake</b>{conf_str}. '
                    f'Scam-like signals detected — verify before applying.</div>',
                    unsafe_allow_html=True)
    elif lbl == "irrelevant":
        st.markdown(f'<div class="alert-irr">⚠️ <b>Irrelevant Posting:</b> The job title '
                    f'and description appear mismatched{conf_str}. This may not be a '
                    f'legitimate opening.</div>', unsafe_allow_html=True)
    elif lbl == "genuine":
        st.markdown(f'<div class="alert-genuine">✅ <b>Verified Genuine Job</b>{conf_str} '
                    f'— classified by PAC model (train_model.py).</div>',
                    unsafe_allow_html=True)
    badge_css = {"genuine":"badge-genuine","fake":"badge-fake","irrelevant":"badge-irr"}.get(lbl,"badge-ai")
    badge_txt = _BADGE.get(lbl, "⏳")
    st.markdown(f"""
    ...
    if user and user["role"] == "seeker":
        if already_applied_dataset(idx, user["id"]):
            ...
            if lbl == "fake":
                st.warning("⚠️ This job is flagged as potentially Fake. Apply with caution.")
...
def page_dashboard_seeker():
    ...
    if section == "📊 Overview":
        ...
        if not profile_text:
            ...
            st.markdown(f"""
            <div class="ai-box">
              🤖 <b>AI Recommendations (PAC + TF-IDF):</b> Top {len(recs)} jobs matching your profile.<br>
              <small style="opacity:0.7;">Fraud-detection: pac_model.pkl via train_model.py · Matching: tfidf_vectorizer.pkl via tfidf_features.py</small>
            </div>
            """, unsafe_allow_html=True)
            for i, (orig_idx, row) in enumerate(recs.iterrows()):
                pct     = max(5, min(99, int(row["score"]*100)))
    ...
    if section == "📊 Overview":
        ...
        if sub:
            if not title or not desc: st.error("Title and description are required.")
                ...
                if ml_lbl == "genuine":
                    st.success(f"✅ Job posted! AI Verdict: **Genuine** ({ml_conf:.0f}%). Your listing is live.")
                elif ml_lbl == "fake":
                    st.error(f"🚨 Posted but flagged **Fake** ({ml_conf:.0f}%). Scam-like language detected — review your posting.")
                elif ml_lbl == "irrelevant":
                    st.warning(f"⚠️ Posted but classified **Irrelevant** ({ml_conf:.0f}%). Title and description may be mismatched.")
                else:
